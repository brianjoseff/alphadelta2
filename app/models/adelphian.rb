class Adelphian < ActiveRecord::Base
  ...
  def prod_s3_credentials
     {:bucket => "alphadelta-pro", :access_key_id => ENV['S3_KEY'], :secret_access_key => ENV['S3_SECRET'] }
  end
  def s3_credentials
      {:bucket => "alphadelta-dev", :access_key_id => ENV['S3_KEY'], :secret_access_key => ENV['S3_SECRET']}
  end
end
